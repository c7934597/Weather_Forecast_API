def createDF():
    ...
    dates = []  # 存日期
    days = []  # 存星期幾
    url = 'http://www.cwb.gov.tw/V7/forecast/town368/3Hr/6401100.htm'  # 小港區三日預報
...
def writeMySql():
    global df
    try:
        conn = pymysql.connect('localhost', port=3306, user='c7934597', passwd='password', charset='utf8',
                               db='weather')  # 連結資料庫
        cursor = conn.cursor()
    except:
        print('資料庫連結錯誤！')
    ...
    for i in df.index:  # 逐行處理
        ...
        if exist == 0:  # 若資料不存在
            cursor.execute(
                'insert into threeday(日期時間,星期,天氣狀況,溫度,體感溫度,蒲福風級,風向,相對溼度,降雨機率,舒適度) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
                tuple(df.loc[i]))  # 將資料加入資料庫
